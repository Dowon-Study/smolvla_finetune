...
def load_policy(args, device):
    ...
    print(f"  학습 파라미터 : {sum(p.numel() for p in policy.parameters() if p.requires_grad)/1e6:.2f}M")
...
def main():
    ...
    for step in range(1, args.steps + 1):
        ...
        if is_main:
            # ⭐ [여기 수정 완료] optimizer.param_groups 인덱싱 추가
    ...
    if is_main: print("\n🎉 모든 관문을 통과하고 학습을 완료했습니다!"); logger.wandb.finish() if args.wandb else None
